...
public class CashOutPanel : SnowUIPlace
{
    ...
    private void Start()
    {
        CashOutManager.FarBefriend()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { FirstUIMode(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = ToeBoldLeg.instance.CashOut_Data.MoneyName;
        InfoText.text = ToeBoldLeg.instance.CashOut_Data.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = ToeBoldLeg.instance.SnowPit;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = ToeBoldLeg.instance.SnowPit;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.FarBefriend().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.FarBefriend().Account;
        Confirm_AccountText.text = CashOutManager.FarBefriend().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.FarBefriend().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = ToeBoldLeg.instance.SnowPit + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = ShedLineRancher.FarGray("CashOut_PolicyAgree");
    ...
    {
        base.Hidding();
        CashOutManager.FarBefriend().WaitToLeafInuit1304();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.FarBefriend().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.FarBefriend().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.FarBefriend().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            GroomRancher.FarBefriend().BindGroom("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            GroomRancher.FarBefriend().BindGroom(AccountErrorText.text);
            return;
        }

        CashOutManager.FarBefriend().Account = AccountInput.text;
        ShedLineRancher.CudStench("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            GroomRancher.FarBefriend().BindGroom("Please agree to the policy");
            return;
        }
        ShedLineRancher.CudGray("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.FarBefriend().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.FarBefriend().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.FarBefriend().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(ToeBoldLeg.instance.CashOut_Data.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.FarBefriend().Data.Cash;
    ...
    {
        if (CashOutManager.FarBefriend().Data.TaskData != null && CashOutManager.FarBefriend().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.FarBefriend().Data.TaskData.Name == "Cash" && CashOutManager.FarBefriend().Data.Cash < CashOutManager.FarBefriend().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.FarBefriend().Data.TaskData.Description, CashOutManager.FarBefriend().Data.Cash, CashOutManager.FarBefriend().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.FarBefriend().Data.TaskData.Name != "Cash" && CashOutManager.FarBefriend().Data.TaskData.NowValue < CashOutManager.FarBefriend().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.FarBefriend().Data.TaskData.Description, CashOutManager.FarBefriend().Data.TaskData.NowValue, CashOutManager.FarBefriend().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.FarBefriend().Data.Record == null || CashOutManager.FarBefriend().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.FarBefriend().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.FarBefriend().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    void ShowGuide()
    {
        ...
        GuideTitleText.text = "";
        GuideTitleText.DOFade(0, .5f).OnComplete(() => {
            if (Titles != null && GuideStep >= 0 && GuideStep < Titles.Length)
                GuideTitleText.text = Titles[GuideStep];
        });
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            GroomRancher.FarBefriend().BindGroom("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        WhimInuitRemove.FarBefriend().LeafInuit("1301", LeftTimeText.text, CashOutManager.FarBefriend().Money.ToString());
    }
}
