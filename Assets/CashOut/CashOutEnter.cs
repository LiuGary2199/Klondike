...
public class CashOutEnter : MonoBehaviour
{
    ...
    void Start()
    {
        CashOutManager.FarBefriend()._CashOutEnter = this;
        OpenPanelBtn.onClick.AddListener(() =>
        {
            UIRancher.FarBefriend().BindUIPlace(nameof(CashOutPanel));
            UIRancher.FarBefriend().FarSwearSoStew(nameof(InferSwear))?.GetComponent<InferSwear>().Leaf(false);
        });
        UpdateMoney();
    }
    ...
    public void UpdateMoney()
    {
        ...
        MoneyTextAnim = DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), CashOutManager.FarBefriend().Money, 1f);
