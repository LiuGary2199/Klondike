...
public class CashOutManager : TiltDigestion<CashOutManager>
{
    [Header("登录平台")]
    ...
    [HideInInspector] public string TrackerSource; // 归因渠道
    long FirstCheckSourceTime;
    long LastCheckSourceTime;

    #region 游戏逻辑
    private void Start()
    {
        Account = ShedLineRancher.FarStench("CashOut_Account");
        Money = ShedLineRancher.FarReach("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {ToeBoldLeg.instance.CashOut_Data.MoneyName} have been converted,Please check your rewards!";
            ConstructionRancher.Befriend.HeavyConstruction();
            ConstructionRancher.Befriend.AllegoryConstruction(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                ConstructionRancher.Befriend.AllegoryConstruction(title, info, (int)Seconds + (i * 10800));
        }
    ...
    void TimeCount() //计时
    {
        if (Data != null)
        {
            ...
            //任务
            if (ToeBoldLeg.instance.CashOut_Data != null && ToeBoldLeg.instance.CashOut_Data.TaskList.Count > 0)
            {
                //记录第一次登录日期utc
                ...
                if (Day >= ToeBoldLeg.instance.CashOut_Data.TaskList.Count) //一天一任务 天数超出任务数量显示默认任务
                    Data.TaskData = ToeBoldLeg.instance.CashOut_Data.TaskList.FirstOrDefault(t => t.IsDefault);
                else
                    Data.TaskData = ToeBoldLeg.instance.CashOut_Data.TaskList[Day];
    ...
    {
        Money += Value;
        ShedLineRancher.CudReach("CashOut_Money", Money);
        _CashOutPanel?.UpdateMoney();
        _CashOutEnter?.UpdateMoney();
    }

    public void WaitToLeafInuit1304() //等待 发送关闭商店后行为1304事件
    {
    ...
    public void LeafInuit1304() ////打点 关闭商店后行为
    {
        CancelInvoke(nameof(Count1304Time));
        if (Event_1304Time <= 0)
            return;
        WhimInuitRemove.FarBefriend().LeafInuit("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", ShedLineRancher.FarStench("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        {
            Platform = "Android";
            DeviceAdId = ShedLineRancher.FarStench("gaid");
        ...
        else
        {
            Platform = "iOS";
            Manufacturer = "Apple";
            DeviceAdId = ShedLineRancher.FarStench("idfv");
        }
        ...
        bool hasuuidAndAppid = ShedLineRancher.FarGray("UuidAndAPPid");
        ...
        ToeKeelRancher.FarBefriend().TineWhimAmid(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        bool isNewPlayer = !PlayerPrefs.HasKey(CAdjoin.Or_UpGunVictim + "Bool") || ShedLineRancher.FarGray(CAdjoin.Or_UpGunVictim);
                        if (isNewPlayer)
                            ShedLineRancher.CudGray("UuidAndAPPid", true);
#endif

                        //刷新token 获取提现规则
                        ShedLineRancher.CudStench("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(ShedLineRancher.FarStench("CashOut_ConvertTime"));
                        ...
                        ShedLineRancher.CudStench("CashOut_ConvertTime", Data.ConvertTime.ToString());
                        ...
                        //归因渠道
                        if (PlayerPrefs.HasKey("FirstCheckSourceTime"))
                            FirstCheckSourceTime = long.Parse(PlayerPrefs.GetString("FirstCheckSourceTime"));
                        if (FirstCheckSourceTime == 0) //第一次检查归因渠道时间
                        {
                            FirstCheckSourceTime = DateTime.Now.Ticks;
                            PlayerPrefs.SetString("FirstCheckSourceTime", FirstCheckSourceTime.ToString());
                        }
                        GetTrackerSource();
                        TrackerSource = PlayerPrefs.GetString("TrackerSource");
                        if (string.IsNullOrEmpty(TrackerSource))
                            GetTrackerSource();
                        else
                            AdSceneManager.Instance.SetupSDK();
                        //测试模式可直接初始化
                        if (AdSceneManager.Instance.IsTest)
                            AdSceneManager.Instance.SetupSDK();
                    }
                    else
                    {
                        ...
                        GroomRancher.FarBefriend().BindGroom("Login fail :" + response.msg);
                    }
                }
                catch (Exception e)
                {
            ...
            fail: () =>
            {
                ...
                GroomRancher.FarBefriend().BindGroom("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        ToeKeelRancher.FarBefriend().TineFar(
            url: url,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = ShedLineRancher.FarEmbryo("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                WhimInuitRemove.FarBefriend().LeafInuit("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        ShedLineRancher.CudEmbryo("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            GroomRancher.FarBefriend().BindGroom($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        ToeKeelRancher.FarBefriend().TineWhimAmid(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    GroomRancher.FarBefriend().BindGroom("Withdraw fail :" + e.Message);
    ...
    {
        WhimInuitRemove.FarBefriend().LeafInuit("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = ShroudDeafRancher.Instance.FarShroudYawn();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    {
        if (string.IsNullOrEmpty(ShedLineRancher.FarStench("CashOut_Token")))
        {
    ...
    public void AddTaskValue(string Name, float Value) //增加任务值
    {
        if (Data.TaskData != null && Data.TaskData.Name == Name)
        {
            ...
            {
                WhimInuitRemove.FarBefriend().LeafInuit("1305", Name, NewValue.ToString(), Data.TaskData.IsDefault.ToString());
    ...
    #endregion
    public void GetTrackerSource() // 获取归因渠道
    {
        string url = $"{BaseUrl}/user/tracker";
        ToeKeelRancher.FarBefriend().TineFar(
         url,
         (result) =>
         {
             try
             {
                 var response = JsonMapper.ToObject<Response_Tracker>(result.downloadHandler.text);
                 if (response.code == 0) // 成功状态码
                 {
                     TrackerSource = response.data.source;
                     PlayerPrefs.SetString("TrackerSource", TrackerSource);
                     CashOutLog("成功获取归因渠道： " + TrackerSource, false, true);
                     ...
                     if (!string.IsNullOrEmpty(TrackerSource))
                     {
                         AdSceneManager.Instance.SetupSDK();
                         //上报用时
                         int TotalTime = (int)TimeSpan.FromTicks(DateTime.Now.Ticks - FirstCheckSourceTime).TotalSeconds; //实际用时
                         int Time30 = (TotalTime / 30) + 1;  //用了几个30秒
                         WhimInuitRemove.FarBefriend().LeafInuit("4010", Time30.ToString(), TotalTime.ToString(), TrackerSource); //上报获取归因渠道成功
                     }
                     else
                     {
                         print(TimeSpan.FromTicks(DateTime.Now.Ticks - FirstCheckSourceTime));
                         if (TimeSpan.FromTicks(DateTime.Now.Ticks - FirstCheckSourceTime) > TimeSpan.FromMinutes(30))
                         {
                             TrackerSource = "Null";
                             PlayerPrefs.SetString("TrackerSource", TrackerSource);
                             WhimInuitRemove.FarBefriend().LeafInuit("4011");
                             CashOutLog("距离初次检查超过30分钟 永不再检查归因渠道", false, true);
                             return;
                         }

                         Invoke(nameof(GetTrackerSource), 31f);
                     }
                     LastCheckSourceTime = DateTime.Now.Ticks;
                 }
                 else
                 {
                     CashOutLog($"获取归因渠道失败: {response.msg}", true);
                 }
             }
             catch (Exception e)
             {
                 CashOutLog($"解析获取归因渠道响应数据失败: {e.Message}", true);
             }
         },
         fail: () =>
         {
             CashOutLog("获取归因渠道请求失败", true);
         },
         timeout: 3f,
         headers: Headers()
        );
    ...
    // 归因渠道
    [System.Serializable]
    public class Response_Tracker : BaseResponse
    {
