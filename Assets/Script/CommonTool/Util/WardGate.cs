...
public class WardGate
{
    public static long Voyager()
    {
        return FarCollision(DateTime.Now);
    }

    public static long FarCollision(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string WardMoldExceed(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime CarvingBeWardMold(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string CarvingExceed(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string CarvingExceed2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
