...
public class GoldAmazonRedeemPanel : BaseUIForms
{
    ...
    public override void Display(object uiFormParams)
    {
        ...
        // 打点
        PostEventScript.GetInstance().SendEvent("1302", NumberUtil.DoubleToStr(SOHOShopManager.instance.getGoldAction()), NumberUtil.DoubleToStr(SOHOShopManager.instance.getAmazonAction()));
    ...
    private void refreshList()
    {
        ...
        double gold = SOHOShopManager.instance.getGoldAction();
        double amazon = SOHOShopManager.instance.getAmazonAction();
        double cash = SOHOShopManager.instance.getCashAction();
        for (int i = 0; i < GoldRedeemManager.instance.goldRedeemGroup.Length; i++)
        {
            ...
            if (item.state != Redeem.RedeemState.Complete)
            {
                ...
                obj.GetComponent<GoldAmazonRedeemItem>().InitItem(item, gold, amazon, cash, i);
            }
        }
    }
    ...
    private void openRecordPanel()
    {
        ...
        panel.GetComponent<RecordPanel>().InitData("gold");
    }
