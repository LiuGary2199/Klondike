...
public class RedeemGiftPanel : BaseUIForms
{
    ...
    void Start()
    {
        ...
        RandomButton.onClick.AddListener(() => {
            if (randomPuzzle != null)
            {
                ...
                {
                    SOHOShopManager.instance.AddRewardPuzzle(randomPuzzle);
                    refreshList();
                }, "110");
    ...
    private void refreshList()
    {
        ...
        Container.GetComponent<RectTransform>().sizeDelta = new Vector2(Container.GetComponent<RectTransform>().sizeDelta.x, 236 * (SOHOPuzzleManager.instance.puzzleShopGroup.Length + 1) / (CommonUtil.IsPortrait() ? 3 : 5));
    }
}
