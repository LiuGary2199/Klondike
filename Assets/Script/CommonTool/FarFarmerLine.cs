...
{
    public static void Moist<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class FarFarmerLine : TiltDigestion<FarFarmerLine>
{
    // Start is called before the first frame update
    public bool TeleMillet;
    public bool ByLife;

    static public FarFarmerLine instance;
    ...
    /// <returns></returns>
    public Vector3 HueNichePerry(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject FarArtisanKickPepper(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float PerryBeSight(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Sight180Be360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int FarServeDirection(Vector2 p1, Vector2 p2)
    {
        float angle = Sight180Be360(PerryBeSight(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 HueRoadwayGlenCity(Renderer Renderer)
    {
    ...
    public Vector2 HueRoadwayGlenCity(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 HueReviewCity(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 HueFloorCitation()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = HueDecadeAgree();
        float screenH = HueDecadeMilieu();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int HueRevertDodge(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] FarSalineInstinct(int total, int count)
    {
    ...
    public Dictionary<string, object> StenchBeOccupation(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string OccupationBeStench(Dictionary<string, object> Info)
    {
    ...
    public int TipWardCope(string dateStart, DateTime dateEnd)
    {
    ...
    public int SecWardCope(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan WardCope(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 FarSumAtMoss(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = FarSumAtMoss(canvas, text, strFragmentIndex);
            Vector3 lastPos = FarSumAtMoss(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = FarSumAtMoss(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 FarSumAtMoss(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float EvidenceByPeck(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Prey= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string AnotherDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string FactoryDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Prey;
