...
public class ADManager : MonoBehaviour
{
    ...
    private void OnRewardedAdRevenuePaidEvent(string adUnitId, MaxSdkBase.AdInfo info)
    {
        ...
        if (!string.IsNullOrEmpty(adjustAdid))
        {
            ...
            print("激励视频  发回收入数据给Mintegral:  adjustAdid:" + adjustAdid);
#endif
        }
    }
    ...
    private void OnInterstitialRevenuePaidEvent(string adUnitId, MaxSdkBase.AdInfo info)
    {
        ...
        if (!string.IsNullOrEmpty(adjustAdid))
        {
            ...
            mBridgeRevenueParamsEntity.SetMaxAdInfo(info);
            MBridgeRevenueManager.Track(mBridgeRevenueParamsEntity);
            print("插屏  发回收入数据给Mintegral:  adjustAdid:" + adjustAdid);
#endif
        }
    }
    ...
    private void RepeatMethod()
    {
        ...
        else
        {
            ...
            if (counter101 >= relax_interval && !pauseTimeInterstitial)
            {
                ...
                // if (!CommonUtil.IsApple() && interstitialVideoReady)
                // {
                //     Time.timeScale = 0;
                //     counter101 = 0;
                //     InterstitialAdTipPanel interstitialAdTipPanel = UIManager.GetInstance().ShowUIForms("InterstitialAdTipPanel") as InterstitialAdTipPanel;
                //     RewardData rewardData = MainManager.Instance.GetGameRewardData_ByWeight(MainManager.Instance._GameData.addatalist, false);
                //     interstitialAdTipPanel.Init(rewardData);
                //     TimeManager.GetInstance().Delay_RealTime(2, () =>
                //     {
                //         UIManager.GetInstance().CloseOrReturnUIForms(interstitialAdTipPanel.name);
    ...
    private void AfterAdPlaySuccess(ADType adType)
    {
        ...
        {
            PostEventScript.GetInstance().SendEvent(interstitialType.ToString());  //打点 插屏广告
            // 计数器清零
            if (interstitialType == 101)
            {
                ...
                //     MainManager.Instance.GetReward(Rewards, new Vector3(0, 0, 1));
                //     GamePanel gamePanel = UIManager.GetInstance().GetPanelByType<GamePanel>();
